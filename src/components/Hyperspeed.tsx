...
type EffectOptions = {
  ...
  distortion?: keyof typeof distortions | 'turbulentDistortion' | string;
  length: number;
  roadWidth: number;
  islandWidth: number;
  lanesPerRoad: number;
  ...
  // Optional flag from original snippet
  isHyper?: boolean;
};
...
const Hyperspeed = ({
  ...
  useEffect(() => {
    ...
    class App {
      ...
      constructor(container: HTMLElement, options: EffectOptions) {
        ...
        if ((this.options as any).distortion == null) {
          (this.options as any).distortion = {
        ...
        // Ensure canvas is not transparent so background is not white
        ...
        // Desactivar fog para evitar complicaciones de compilación y oscurecimiento
      ...
      onTouchStart(ev: any) { this.onMouseDown(ev); }
      onTouchEnd(ev: any) { this.onMouseUp(ev); }
    ...
    class CarLights {
      webgl: App; options: EffectOptions; colors: any; speed: [number, number]; fade: THREE.Vector2; mesh!: THREE.Mesh;
      constructor(webgl: App, options: EffectOptions, colors: any, speed: [number, number], fade: THREE.Vector2) { this.webgl = webgl; this.options = options; this.colors = colors; this.speed = speed; this.fade = fade; }
      init() {
        const options = this.options;
        const curve = new THREE.LineCurve3(new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, 0, -1));
        const geometry = new THREE.TubeGeometry(curve, 40, 1, 8, false);
        const instanced = new THREE.InstancedBufferGeometry().copy(geometry); instanced.instanceCount = options.lightPairsPerRoadWay * 2;
        const laneWidth = options.roadWidth / options.lanesPerRoad;
        const aOffset: number[] = []; const aMetrics: number[] = []; const aColor: number[] = [];
        let colors = this.colors; colors = Array.isArray(colors) ? colors.map((c: number) => new THREE.Color(c)) : new THREE.Color(colors);
        for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
          const radius = random(options.carLightsRadius); const length = random(options.carLightsLength); const speed = random(this.speed);
          ...
          const offsetY = random(options.carFloorSeparation) + (radius as number) * 1.3; const offsetZ = -random(options.length);
        ...
        const material = new THREE.ShaderMaterial({ fragmentShader: carLightsFragment, vertexShader: carLightsVertex, transparent: true, uniforms: Object.assign({ uTime: { value: 0 }, uTravelLength: { value: options.length }, uFade: { value: this.fade } }, this.webgl.fogUniforms, (this.options as any).distortion.uniforms) });
        material.onBeforeCompile = (shader) => { shader.vertexShader = shader.vertexShader.replace('#include <getDistortion_vertex>', (this.options as any).distortion.getDistortion); };
        const mesh = new THREE.Mesh(instanced, material); (mesh as any).frustumCulled = false; this.webgl.scene.add(mesh); this.mesh = mesh;
      }
    ...
    const carLightsVertex = `
      ...
      void main(){ vec3 transformed = position.xyz; float radius = aMetrics.x; float myLength = aMetrics.y; float speed = aMetrics.z; transformed.xy *= radius; transformed.z *= myLength; transformed.z += myLength - mod(uTime * speed + aOffset.z, uTravelLength); transformed.xy += aOffset.xy; float progress = abs(transformed.z / uTravelLength); transformed.xyz += getDistortion(progress); vec4 mvPosition = modelViewMatrix * vec4(transformed, 1.); gl_Position = projectionMatrix * mvPosition; vUv = uv; vColor = aColor; }
    `;
    ...
    class LightsSticks {
      ...
      init() {
        ...
        const instanced = new THREE.InstancedBufferGeometry().copy(geometry); const totalSticks = options.totalSideLightSticks; instanced.instanceCount = totalSticks;
        const stickoffset = options.length / (totalSticks - 1);
        const aOffset: number[] = []; const aColor: number[] = []; const aMetrics: number[] = [];
        let colors: any = options.colors.sticks; colors = Array.isArray(colors) ? colors.map((c: number) => new THREE.Color(c)) : new THREE.Color(colors);
        for (let i = 0; i < totalSticks; i++) {
          const width = random(options.lightStickWidth); const height = random(options.lightStickHeight); aOffset.push((i - 1) * stickoffset * 2 + stickoffset * Math.random());
          const color = pickRandom(colors); aColor.push(color.r, color.g, color.b); aMetrics.push(width as number, height as number);
        }
        instanced.setAttribute('aOffset', new THREE.InstancedBufferAttribute(new Float32Array(aOffset), 1, false));
        instanced.setAttribute('aColor', new THREE.InstancedBufferAttribute(new Float32Array(aColor), 3, false));
        instanced.setAttribute('aMetrics', new THREE.InstancedBufferAttribute(new Float32Array(aMetrics), 2, false));
        const material = new THREE.ShaderMaterial({ fragmentShader: sideSticksFragment, vertexShader: sideSticksVertex, side: THREE.DoubleSide, uniforms: Object.assign({ uTravelLength: { value: options.length }, uTime: { value: 0 } }, (this.options as any).distortion.uniforms) });
        material.onBeforeCompile = (shader) => { shader.vertexShader = shader.vertexShader.replace('#include <getDistortion_vertex>', (this.options as any).distortion.getDistortion); };
        const mesh = new THREE.Mesh(instanced, material); (mesh as any).frustumCulled = false; this.webgl.scene.add(mesh); this.mesh = mesh;
      }
      update(time: number) { (this.mesh.material as THREE.ShaderMaterial).uniforms.uTime.value = time; }
    ...
    const sideSticksFragment = `
      varying vec3 vColor; void main(){ vec3 color = vec3(vColor); gl_FragColor = vec4(color,1.); }
    `;

    class Road {
      webgl: App; options: EffectOptions; uTime = { value: 0 }; leftRoadWay!: THREE.Mesh; rightRoadWay!: THREE.Mesh; island!: THREE.Mesh;
      constructor(webgl: App, options: EffectOptions) { this.webgl = webgl; this.options = options; }
      createPlane(side: number, width: number, isRoad: boolean) {
        const options = this.options; const geometry = new THREE.PlaneGeometry(isRoad ? options.roadWidth : options.islandWidth, options.length, 20, 100);
        ...
        const material = new THREE.ShaderMaterial({ fragmentShader: isRoad ? roadFragment : islandFragment, vertexShader: roadVertex, side: THREE.DoubleSide, uniforms: Object.assign(uniforms, (this.options as any).distortion.uniforms) });
        material.onBeforeCompile = (shader) => { shader.vertexShader = shader.vertexShader.replace('#include <getDistortion_vertex>', (this.options as any).distortion.getDistortion); };
    ...
    const roadVertex = `
      uniform float uTime; uniform float uTravelLength; varying vec2 vUv; #include <getDistortion_vertex>
    ...
    (function () {
      ...
      // map string name to distortion object if provided
      // @ts-expect-error dynamic
  ...
  return <div id="lights" style={{ width: '100%', height: '100%', position: 'absolute', inset: 0, overflow: 'hidden' }} />;
};
